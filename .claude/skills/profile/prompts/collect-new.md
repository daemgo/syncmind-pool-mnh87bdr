...
信息不足时，基于企业大致行业/规模做合理假设，所有假设型结论必须标注 “[假设待验证]”。
...
不足信息时可以结合 `{industry_pain_points}` 和招聘/新闻做合理推断，并标记 “[假设待验证]”。
...
- 输出不超过 5 条 `painPoints`：每条包含 `area`（痛点领域）、`description`（一句话描述）、`severity`（高/中/低 或 类似分级），必要时标注 “[假设待验证]”；
